        ...
        <div class="m-4 sm:mx-[150px]">
            <form action="{{ route('submit_details') }}" method="post">
                ...
                <div class="flex w-full inline-flex items-center my-2">
                    ...
                    <label for="Adults" class="w-3/5">{{ session()->get('venue_details')[2]['quantity'] }} pax</label>
                </div>
                @endif
                @if (count(session()->get('venue_details')[3]) != 0)
                <div class="flex w-full inline-flex items-center my-2">
                    <label for="Adults" class="mx-4 w-2/5">Elderly & kids</label>
                    <label for="Adults" class="w-3/5">{{ session()->get('venue_details')[3]['quantity'] }} pax</label>
                </div>
                @endif
                @if (count(session()->get('venue_details')[4]) != 0)
                <div class="flex w-full inline-flex items-center my-2">
                    <label for="Adults" class="mx-4 w-2/5">Group</label>
                    <label for="Adults" class="w-3/5">{{ session()->get('venue_details')[4]['quantity'] }} pax</label>
                </div>
                @endif
                <div class="flex w-full inline-flex items-center my-2">
                ...
                @foreach ($existing_customers as $item)
                <div class="flex w-full inline-flex items-center my-2">
                    <label for="Adults" class="mx-4 w-2/5">Customer name</label>
                    <input type="text" name="customer_details[]" value="{{ $item->customer_name }}"
                        class="bg-gray-50 w-3/5 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required>
                </div>
                <div class="flex w-full inline-flex items-center my-2">
                    <label for="Adults" class="mx-4 w-2/5">Phone no.</label>
                    <input type="text" name="customer_details[]" value="{{ $item->customer_phone }}"
                        class="bg-gray-50 w-3/5 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required>
                </div>
                @endforeach
                @for ($i = 0; $i < $total_customer_after; $i++) <div class="flex w-full inline-flex items-center my-2">
                    <label for="Adults" class="mx-4 w-2/5">Customer name</label>
