<x-guest-layout>
    <div class="flex flex-col m-4 sm:m-[35px]">
        ...
        <div class="m-4 sm:mx-[150px]">
            <form action="{{ route('update_customer', ['order_id' => $order->id]) }}" method="post">
                @csrf
                @method('post')
                ...
                <div class="flex w-full inline-flex items-center my-2">
                    ...
                    <select name="venue"
                        ...
                        <option value="{{ $item->id }}" {{ $item->status == 'Sold out' ? 'disabled' : '' }} {{
                            $order->venue_id == $item->venue_id ? 'selected' : ''}}>{{
                ...
                <div class="flex w-full inline-flex items-center my-2">
                    ...
                    <select name="adults"
                        ...
                        <option value="{{ $number }}" {{ $adult==$number ? 'selected' : '' }}>{{ $number }}</option>
                        }
                        @endforeach
                    </select>
                </div>
                <div class="flex w-full inline-flex items-center my-2"><label for="adult" class="mx-4 w-2/5">Elderly &
                    ...
                    <select name="kids"
                        ...
                        required>
                        @foreach (range(0, 29) as $number) {
                        <option value="{{ $number }}" {{ $kids==$number ? 'selected' : '' }}>{{ $number }}</option>
                        }
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex w-full inline-flex items-center my-2">
                    ...
                    <select name="group"
                        ...
                        <option value="{{ $number }}" {{ $group==$number ? 'selected' : '' }}>{{ $number }}</option>
                        }
                        @endforeach
                    </select>
